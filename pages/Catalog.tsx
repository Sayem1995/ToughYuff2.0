...
const Catalog: React.FC<CatalogProps> = ({ products, brands = [], categories = [] }) => {
  ...
  // TEMPORARY DB SEED SCRIPT FOR TYSON 30K
  React.useEffect(() => {
    const seedTyson = async () => {
      try {
        const productsRef = collection(db, 'products');
        const q = query(productsRef, where("brandId", "==", "tyson-30k"));
        const snapshot = await getDocs(q);

        if (snapshot.size === 0) {
          console.log("[Seed] Tyson 30K not found in DB. Seeding...");
          const tysonProducts = INITIAL_PRODUCTS.filter(p => p.brandId === 'tyson-30k');
          ...
          // Also seed the brand
          const mtBrand = BRANDS.find(b => b.id === 'tyson-30k');
          if (mtBrand) {
          ...
          console.log("[Seed] Tyson 30K already found in DB. Skipping.");
        }
      } catch (error) {
