...
const Catalog: React.FC<CatalogProps> = ({ products, brands = [], categories = [] }) => {
  ...
  // TEMPORARY DB SEED SCRIPT FOR YME 5% and YME 0%
  React.useEffect(() => {
    const seedYME = async () => {
      try {
        const brandsToSeed = ['yme-nic', 'yme-nonic'];

        for (const targetBrandId of brandsToSeed) {
          ...
          if (snapshot.size === 0) {
            ...
            const ymeProducts = INITIAL_PRODUCTS.filter(p => p.brandId === targetBrandId);
            for (const product of ymeProducts) {
            ...
            // Also seed the brand
            const ymeBrand = BRANDS.find(b => b.id === targetBrandId);
            if (ymeBrand) {
              const brandRef = doc(db, 'brands', ymeBrand.id);
              await setDoc(brandRef, ymeBrand);
              console.log(`[Seed] Seeded brand ${ymeBrand.name}`);
            }
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-x-4 gap-y-8">
              {filteredProducts.map((product) => (
                (product.category === 'thc-disposables' || product.category === 'thc-cartridges') ? (
                  ...
                  <Link to={`/product/${product.id}`} key={product.id} className="group relative bg-transparent rounded-2xl transition-all duration-300 hover:-translate-y-1 flex flex-col overflow-hidden">
                    {/* Admin Controls */}
                    {isAdmin && (
