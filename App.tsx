...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const fetchAndComputeBrands = async () => {
      ...
      // Merge logic: Combine active static brands and dynamic brands
      // We want dynamic brands to *override* static brands if they share an ID or Name
      const brandMap = new Map();
      const nameMap = new Map();

      // First, load static brands
      activeStaticBrands.forEach((b: any) => {
        const nameKey = (b.name || '').toLowerCase().trim();
        brandMap.set(b.id, { ...b });
        nameMap.set(nameKey, b.id);
      });

      // Second, apply dynamic brand overrides
      dynamicBrands.forEach((b: any) => {
        const nameKey = (b.name || '').toLowerCase().trim();
        const existingId = nameMap.get(nameKey) || b.id;
        ...
        if (master) {
          ...
          if (b.category) master.category = b.category;
        } else {
          // Completely new dynamic brand
          brandMap.set(b.id, b);
          nameMap.set(nameKey, b.id);
        }
      });

      const unique = Array.from(brandMap.values());
      // Sort alphabetically by name
